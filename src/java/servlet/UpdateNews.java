...
//        int id = u.getId();
        ...
        List<Category> listDM = dao2.getAllCater();
        request.setAttribute("listDM", listDM);
        request.getRequestDispatcher("editBaiviet.jsp").forward(request, response);
    }
...
//        processRequest(request, response);
        ...
        boolean valid = false;
        NewsDAO dao = new NewsDAO();
        valid = dao.updateNews(name, image, description, detail, caterId, id);
        if(valid){
            response.sendRedirect("ShowNews");
        }
//            
        else{
            request.setAttribute("alert", caterId);
